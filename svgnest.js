...
(function (root) {
	...
	function SvgNest() {
		...
		var nfpCache = {};
		var config = {
			clipperScale: 10000000,	//clipperjs库 计算整数，坐标需要先整数化
			curveTolerance: 0.3,
			spacing: 0,
			rotations: 4,
			populationSize: 10,
			...
			function offsetTree(t, offset, offsetFunction) {
				for (var i = 0; i < t.length; i++) {
					...
					if (offsetpaths.length == 1) {
						// replace array items in place  下面的意思是 删除t内的元素 换成 offsetpaths[0]
		...
		this.launchWorkers = function (tree, binPolygon, config, progressCallback, displayCallback) {
			...
			if (GA === null) {
				...
				// seed with decreasing area 面积升序
			...
			for (i = 0; i < GA.population.length; i++) {
				if (!GA.population[i].fitness) {	//若有个体没有 fitness值
					individual = GA.population[i];
					break;
				}
			}

			//如果没有不适应的 那就下一代，指定第二组 为观察对象
			if (individual === null) {
			...
			var nfpPairs = [];	//nfpCache 中还不存在的 nfp对
			var key;
			var newCache = {};

			//下面 将构造每一个形状 与 其他形状的 nfp 对元组信息
			for (i = 0; i < placelist.length; i++) {
				var part = placelist[i];
				//构造一个 边框和 形状 对的 KEY {A,B,inside,Arotation,Brotation}
				key = { A: binPolygon.id, B: part.id, inside: true, Arotation: 0, Brotation: rotations[i] };


				//下面 从  nfpCache 转移到 newCache
				//如果 nfpCache 没有这一对记录，那就加入 nfpPairs
				...
				//将每一个前面的 形状 都和 当前形状 做组合 构造key 尝试转移到 newCache，没有就 加入 nfpPairs
				for (j = 0; j < i; j++) {
			...
			//这里 利用一个 劫持内函数 来 更新 progress。该函数在p.map内每次切换时被调用
			...
			p.map(function (pair) {
				...
				if (pair.key.inside) {
					...
					if (searchEdges) {
						...
						//如果 minkowskiDifference 形状包含 原点  说明两个形状有交集
					}
			...
			}).then(function (generatedNfp) {	//计算好 加入 cache
				if (generatedNfp) {
				...
				//将所有paths作为一个单位 构造数组 提交计算，map处理时，只会有一个thread构建
				...
				p2.map(worker.placePaths).then(function (placements) {
					...
					//找出评分最高的 评分怎么算 请看 placementworker.js
	...
	function GeneticAlgorithm(adam, bin, config) {
		...
		// Each individual is a object representing the order of insertion and the angle each part is rotated
		//每一组包括 置入序列 和 旋转序列
		//组织角度数组（可行角度）
		var angles = [];
