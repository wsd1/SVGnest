	...
	this.placePaths = function (paths) {
		...
		while (paths.length > 0) {
			...
			for (i = 0; i < paths.length; i++) {
				...
				//再来确定 和 已经摆好的 pathes之间的 nfp是否已经OK
				var error = false;
				...
				if (placed.length == 0) {
					...
					for (j = 0; j < binNfp.length; j++) {
						for (k = 0; k < binNfp[j].length; k++) {	//边框nfp上的每一个点
							if (position === null || binNfp[j][k].x - path[0].x < position.x) {
								//path[0] 是定位点，找 边框nfp上左侧极值点
								position = {
